...
# 2022.11.13: add edge layer
class WeTr_edge(nn.Module):
    def __init__(self, backbone, num_classes=20, embedding_dim=256, pretrained=None,
                 stride=(4, 2, 2, 2), in_chans=3, pooling="gmp"):
        super().__init__()
        self.num_classes = num_classes
        ...
        self.fc_edge1 = nn.Sequential(
            nn.Conv2d(self.in_channels[0], 32, 1, bias=False),
            nn.GroupNorm(4, 32),
            # nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=False),  # add by yinxcao, 2021.11.06
            nn.ReLU(inplace=True),
        )
        self.fc_edge2 = nn.Sequential(
            nn.Conv2d(self.in_channels[1], 32, 1, bias=False),
            nn.GroupNorm(4, 32),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.ReLU(inplace=True),
        )
        self.fc_edge3 = nn.Sequential(
            nn.Conv2d(self.in_channels[2], 32, 1, bias=False),
            nn.GroupNorm(4, 32),
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False),
            nn.ReLU(inplace=True),
        )
        self.fc_edge4 = nn.Sequential(
            nn.Conv2d(self.in_channels[3], 32, 1, bias=False),
            nn.GroupNorm(4, 32),
            nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False),
            nn.ReLU(inplace=True),
        )
        self.fc_edge5 = nn.Sequential(
            nn.Conv2d(32*4, 64, 1, bias=False),
            nn.GroupNorm(4, 64),
            nn.ReLU(inplace=True),
        )
        self.fc_edge6 = nn.Conv2d(64, 1, 1, bias=True)
        self.edge_layers = nn.ModuleList([self.fc_edge1, self.fc_edge2, self.fc_edge3,
                                          self.fc_edge4, self.fc_edge5, self.fc_edge6])
        # for fusing edge and segmentation
        self.decoder = SegFormerHeadEdge(feature_strides=self.feature_strides, in_channels=self.in_channels,
                                     embedding_dim=self.embedding_dim, num_classes=self.num_classes,
                                    edge_channel=64)
    ...
    def get_param_groups(self):
        ...
        for param in list(self.decoder.parameters()):
            param_groups[2].append(param)
        for param in list(self.edge_layers.parameters()):
            param_groups[2].append(param)
    ...
    def forward(self, x):
        ...
        edge_fea = self.fc_edge5(torch.cat([edge1, edge2, edge3, edge4], dim=1))
        edge_out = F.interpolate(self.fc_edge6(edge_fea), size=(h, w), mode='bilinear', align_corners=False)

        # segmentation
        mask = self.decoder((x1, x2, x3, x4, edge_fea))
        mask = F.interpolate(mask, size=(h, w), mode='bilinear', align_corners=False)

        return mask, edge_out


# 2022.10.3, return cls and seg
class Mitcls_seg(nn.Module):
    def __init__(self, backbone, in_chans=3, num_classes=20,
                 embedding_dim=256, pretrained=None,
                 stride=(4, 2, 2, 1), pooling="max"):
        super().__init__()
        self.num_classes = num_classes
        self.embedding_dim = embedding_dim
        self.feature_strides = [4, 8, 16, 32]
    ...
    def forward(self, x, cam_only=False):
        ...
        # cls
        features = self.classifier(_x[-1])
        cls = self.pooling(features)
        cls = cls.view(-1, self.num_classes)

        # seg
        seg = self.decoder(_x)
        cls_seg = self.pooling(seg)
        cls_seg = cls_seg.view(-1, self.num_classes)
        ...
        return features, cls, seg, cls_seg
...
class Mitcls_puzzle(nn.Module):
    ...
    def forward(self, x, cam_only=False, with_cam=True):
        ...
        if with_cam:
            ...
            logits = self.pooling(features)
            logits = logits.view(-1, self.num_classes) # N 2
            return logits, features
        else:
            x4 = self.pooling(x4)
            logits = self.classifier(x4).view(-1, self.num_classes)
            return logits
...
class Mitcls_seam(nn.Module):
    ...
    def forward(self, x, cam_only=False):
        ...
        f8_3 = F.interpolate(f8_3, (h, w), mode='bilinear', align_corners=True)
...
class Mitcls_CAM_multi4(nn.Module):
    ...
    def forward(self, x, cam_only=False):
        ...
        cls1 = cls1.view(-1, self.num_classes)
        cls2 = cls2.view(-1, self.num_classes)
        cls3 = cls3.view(-1, self.num_classes)
        cls4 = cls4.view(-1, self.num_classes)
